...
def load_pt_data(f):
    ...
    for k, v in temp.items():
        new_temp[k.replace('mixture_', '').replace('prompt_', '').replace('target_', '')] = v
    return new_temp
...
Pretrain_format = {
    'keys': ["raw_audio_seq", "audio_seq"],
    'type': ["audio", "audio"],
    'features': [],
    'loss_key': 'audio_seq',
}
SeparatebyText_format = {
    'keys': ["noise_seq", "text_t5_seq", "audio_seq"],
    'type': ["audio", "text_t5", "audio"],
    'features': [],
    'loss_key': 'audio_seq',
} #similar to Spex_format, but the prompt is text   
task_formats = {
    'lm': lm_format,
    'tts': tts_format,
    'asr': asr_format,
    ...
    'Pretrain': Pretrain_format,
    'SeparatebyText': SeparatebyText_format,
}
