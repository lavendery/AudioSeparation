...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    for part in $valid_set $train_set; do
      ...
      cat data/${TASK}/${part}/wav.scp | shuf >  data/${TASK}/${part}/wav.scp.shuf
      split_scp=
      for n in `seq 1 $ngpu`; do
          split_scp="$split_scp data/${TASK}/${part}/${ngpu}splits/wav.${n}.scp"
      done
      utils/split_scp.pl data/${TASK}/${part}/wav.scp.shuf $split_scp
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    ...
    # for part in $valid_set; do
      ...
      # prompt Audio
      utils/run.pl JOB=1:$ngpu data/${TASK}/${part}/${ngpu}splits/log/wav_codec_dump.JOB.log \
        python3 data_scripts/offline_tokenization.py \
          --input-file data/${TASK}/${part}/${ngpu}splits/wav.JOB.scp \
          --output-file data/${TASK}/${part}/${ngpu}splits/wav_codec.JOB.pt \
          --tokenizer audio --rank JOB || exit 1;
    ...
    for part in $train_set $valid_set; do
      for n in `seq 0 $[$ngpu-1]`; do
        python3 data_scripts/create_data_json.py \
         ...
         --raw_audio_seq  data/${TASK}/${part}/${ngpu}splits/wav_codec.$[$n+1].pt \
         --audio_seq  data/${TASK}/${part}/${ngpu}splits/wav_codec.$[$n+1].pt \
         & 
      done; wait
    done
fi
...
# train_data_jsons="data/Spex_large/test/8splits/data_voicebank.ALL.json"
train_data_jsons="data/audioset/train/32splits/data.ALL.json"
train_data_jsons="$train_data_jsons data/libri-light/train/32splits/data.ALL.json"
...
# valid_data_jsons="data/Spex_large/test/8splits/data_voicebank.ALL.json"
valid_data_jsons="data/audioset/val/32splits/data.ALL.json"
valid_data_jsons="$valid_data_jsons data/libri-light/val/32splits/data.ALL.json"


tag="pretrain_audioset_libri"
...
inference_tag="esc-50-simulation_3s"
